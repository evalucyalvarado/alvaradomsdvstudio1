...
// Correctly define the search query
export const search2 = `Marsh AND unit_code:"NMNHPALEO" AND object_type:"Holotypes"`;

// Array to hold the data
export let myArray2 = [];

// String to hold the stringified JSON data
export let jsonString2 = '';

// Function to fetch search data
export async function fetchSearchData2() {
    const url = `${searchBaseURL}?api_key=${apiKey}&q=${encodeURIComponent(search2)}`;
    ...
    try {
        ...
        // Set jsonString2 to the complete data after all fetches
        jsonString2 = JSON.stringify(myArray2, null, 2); // Make JSON readable
        console.log('Complete data:', myArray2);

        // Note: Do NOT initiate download here automatically.
        // The download should be controlled manually in the main file.
    } catch (error) {
        console.log('Error in fetchSearchData2:', error);
    }
}
